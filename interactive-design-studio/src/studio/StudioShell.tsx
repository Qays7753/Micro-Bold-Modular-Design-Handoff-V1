// Micro Visual System — Studio Chrome: أداة المراجعة (03-SPEC)
...
// مفصول بصريًا ودلاليًا عن واجهة Micro ويوسم بذلك في كل مكان.
        ...
        <div className="studio-chip-row">
          {Object.values(ROUTES).map((r) => (
            <button
              key={r.screenId}
              type="button"
              className={`studio-chip${r.screenId === params.screenId ? ' is-active' : ''}${r.status === 'deferred' ? ' is-deferred' : ''}`}
...
export function ScreenInfoDrawer({
  ...
  const fixtureFile =
    route.screenId === 'OVR-NOW' ? 'ovr-now.json' : route.screenId === 'FIN-OVERVIEW' ? 'fin-overview.json' : route.screenId === 'OPS-SALE-CREATE' ? 'ops-sale-create.json' : '—'
    ...
    <aside className="studio-drawer" role="dialog" aria-label="معلومات الشاشة" aria-modal="false">
      ...
      <div className="studio-drawer__body">
        <dl className="studio-drawer__defs">
          ...
          <div>
            <dt>حالة التنفيذ</dt>
            <dd>{route.status === 'built' ? 'Candidate — منفذة بانتظار مراجعة المالك' : 'Not Started — Deferred'}</dd>
          </div>
          <div>
            <dt>مراجع القرار</dt>
    ...
    <header className="studio-bar">
      <div className="studio-bar__row">
        <div className="studio-bar__identity">
          <MicroQuadStudio />
          <div>
            <h1 className="studio-bar__title">Micro Visual System — V1</h1>
            <p className="studio-bar__sub">Interactive Design Reference Studio · المرجع: MICRO-VISUAL-DESIGN-DECISIONS-V1</p>
          </div>
        </div>
      ...
      {aboutOpen ? (
        <div className="studio-about anim-crossfade">
          ...
          <p className="type-supporting">
            ...
            OPS-SALE-CREATE: {Object.keys(sale.scenarios).length} — كلها عبر مبدّل الحالة أعلاه أو الروابط العميقة.
          </p>
        </div>
      ) : null}
      {children}
