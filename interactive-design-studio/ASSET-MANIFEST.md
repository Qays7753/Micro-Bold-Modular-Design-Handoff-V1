# ASSET-MANIFEST — سجل الأصول والتراخيص (V2 — مراجعة 2026-09-23)

**المرجع:** `13-ASSET-TOKEN-REGISTER.md` + §§9، 13.21، 21.1 + 17-COLOR-DECISION. كل أصل هنا مصدره وترخيصه موثقان، أو موسوم كأصل مرشح/فجوة — لا أصول مجهولة.
...
`public/favicon.svg` (V2): علامة رباعية استوديو-مرجعية **بألوان المنظومة الجديدة** (#D97757 هية · #A94630 فعل · #F0F3F4 سطح · #305968 معلومة) — أداة تشغيل للاستوديو فقط وليست شعار منتج.

## 4. Tokens (V2)

القيم مطابقة لـ`src/foundations/tokens.css` (المصدر الوحيد): الهوية القائدة `#D97757` (اعتماد المالك)، الفعل `#A94630`، Ink `#1D2930`، Canvas `#F0F3F4`… والقيم المشتقة الموثقة (`#C4663F`، `#8F3B27`، `#E4EAEC`، `#8A959C`، `#A6AEB4`). القاموس الكامل: `TOKEN-DICTIONARY.md/.json` (163 Token بثلاث طبقات) مولّد آليًا من المصدر. لا Hex خارج طبقة الـTokens. سجل الجذر `13-ASSET-TOKEN-REGISTER.md` حُدّث بأزواج الجولة الجديدة.

> **تاريخي:** وصف المنظومة القديمة (Deep Ink #171923، Canvas #F4F6FA، Indigo #4F46E5، Citrus #D9F43B) موجود فقط في سجل Git وتقارير الإصدار الأول الموسومة تاريخيًا — غير مستخدم في أي مصدر أو واجهة نشطة (تدقيق نصي وHEX موثق في REVISION-COVERAGE-LEDGER).
