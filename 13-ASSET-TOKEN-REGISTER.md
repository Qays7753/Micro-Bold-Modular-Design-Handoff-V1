# Asset and Token Register — قرار الألوان الجديد مقابل التنفيذ

**محدّث بعد تنفيذ المراجعة V2 (2026-09-23):** المنظومة أدناه **منفذة فعليًا الآن** في المصدر النشط (`src/foundations/tokens.css` هو المصدر الوحيد) ومفحوصة بمتصفح حقيقي — 47/47 فحصًا آليًا + تباين 25/25 من الأزواج الفعلية (`interactive-design-studio/reports/CONTRAST-REPORT.md`). المرجع الحاكم `17-COLOR-DECISION-2026-09-23.md`، وأمر التحديث `18-…`، وبوابة التغطية `19-…` (سجل التنفيذ: `interactive-design-studio/reports/REVISION-COVERAGE-LEDGER.md`).

## خط الأساس الجديد — منفذ في النسخة المراجَعة (V2)

| الدور | القيمة | التنفيذ في V2 |
|---|---|---|
| `Brand/Lead` | `#D97757` | **منفذ** — كتلة بطاقة النتيجة في OVR (بنص Ink) والعلامة الرباعية وFavicon؛ توزيع التكوين مرشح لمراجعة المالك البصرية |
| `Action/Primary` / `OnAction` | `#A94630` / `#FFFFFF` | **منفذ** — الزر الصلب كامل العرض ومضغوطه المشتق `#8F3B27` ولون الاختيار |
| `Brand/Soft` | `#FBE9E2` | **منفذ** — Insight فرصة وAccent و::selection وحساب مفتوح |
| `Text/Primary` / `Text/Secondary` | `#1D2930` / `#53616A` | **منفذ** — Ink فوق الهوية حصرًا |
| `Canvas` / `Surface` | `#F0F3F4` / `#FFFFFF` | **منفذ** |
| `Information` / `Surface` | `#305968` / `#DFEDF1` | **منفذ** — شرح ومعلومة وتركيز ودون اتصال (دور Local موثق ISS-006) |
| `Success` / `Surface` | `#16765A` / `#DFF3E9` | **منفذ** — كلمة «ربح» وإشارة المكتمل |
| `Attention` / `Surface` | `#95590C` / `#FFF0D7` | **منفذ** — دين مستحق/غير موزع |
| `Danger` / `Surface` | `#B0324F` / `#FFE7EB` | **منفذ** — «خسارة» وأخطاء الحقول |
| `Partial/Unknown` / `Surface` | `#5B6770` / `#EDF1F2` | **منفذ** — بطاقات جزئية بشريط جانبي |
| `Boundary/Interactive` / `Divider` | `#78868D` / `#DCE3E5` | **منفذ** — قياس 3.37:1/3.75:1 (غير نصي) |
| قيم مشتقة (17 §2 يفوّض ZAI) | `#C4663F` عمق هوية غير نصي · `#E4EAEC` سطح ثانٍ · `#8A959C` معطل · `#A6AEB4` زخرفي | **منفذة وموثقة** |

## ألوان أساسية — رصد تاريخي للإصدار الأول (لا قيم سارية — أزيلت من المصدر النشط)
...
الأزواج القديمة حُفظت في تاريخ Git؛ **تقرير التباين الحالي** (`interactive-design-studio/reports/CONTRAST-REPORT.md`) محسوب من أزواج النسخة المراجَدة الفعلية (25 زوجًا — كلها PASS). القاموس والوثائق أُعيد توليدها من المصدر الجديد (`TOKEN-DICTIONARY.md/.json`)، وتدقيق نصي وHEX على المصدر والأدلة النشطة يؤكد خلوها من عائلة Indigo/Citrus. Light Mode فقط.
